...
bool32 AI_IsDamagedByRecoil(u32 battler)
{
    ...
    if (ability == ABILITY_MAGIC_GUARD || ability == ABILITY_ROCK_HEAD || ability == ABILITY_SHELL_ARMOR|| ability == ABILITY_BATTLE_ARMOR)
        return FALSE;
    return TRUE;
}
...
enum ItemHoldEffect AI_DecideHoldEffectForTurn(u32 battlerId)
{
    ...
    if (gFieldStatuses & STATUS_FIELD_MAGIC_ROOM)
        return HOLD_EFFECT_NONE;
    if (gAiLogicData->abilities[battlerId] == ABILITY_KLUTZ && !(gStatuses4[battlerId] & STATUS4_GASTRO_ACID))
        return HOLD_EFFECT_NONE;
...
bool32 ShouldSetRain(u32 battlerAtk, u32 atkAbility, enum ItemHoldEffect holdEffect)
{
    ...
    if (holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA
     && (atkAbility == ABILITY_SWIFT_SWIM
      ...
      || HasMoveWithEffect(battlerAtk, EFFECT_BOIL_OVER)
...
bool32 AI_ShouldSetUpHazards(u32 battlerAtk, u32 battlerDef, struct AiLogicData *aiData)
{
    if (aiData->abilities[battlerDef] == ABILITY_MAGIC_BOUNCE
     ...
     || HasMoveWithEffect (battlerDef, EFFECT_MOP)
     || HasMoveWithEffect(battlerDef, EFFECT_WINGS_OF_CORRECTION)
