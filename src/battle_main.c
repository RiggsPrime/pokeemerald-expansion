...
static void SpriteCB_MoveWildMonToRight(struct Sprite *sprite)
{
    ...
    {
        if (B_FAST_INTRO_NO_SLIDE == FALSE && !gTestRunnerHeadless)
            sprite->x2 += 2; // Controls the speed at which wild Pokemon sprite enters the battle scene horizontally (original value of 2)
        else
            sprite->x2 = 0;
...
void SwitchInClearSetData(u32 battler)
{
    ...
    if (effect == EFFECT_BATON_PASS)
    {
        ...
        gStatuses3[battler] &= (STATUS3_LEECHSEED_BATTLER | STATUS3_LEECHSEED | STATUS3_ALWAYS_HITS | STATUS3_PERISH_SONG | STATUS3_ROOTED
                                       | STATUS3_FARADAY_CAGED | STATUS3_EMBARGO | STATUS3_TELEKINESIS | STATUS3_MAGNET_RISE | STATUS3_HEAL_BLOCK
                                       | STATUS3_AQUA_RING | STATUS3_POWER_TRICK);
        gStatuses4[battler] &= (STATUS4_MUD_SPORT | STATUS4_WATER_SPORT | STATUS4_INFINITE_CONFUSION | STATUS4_GASTRO_ACID);
        for (i = 0; i < gBattlersCount; i++)
        {
...
const u8* FaintClearSetData(u32 battler)
{
    ...
    gBattleMons[battler].status2 = 0;
    gStatuses3[battler] = 0;
    gStatuses4[battler] &= STATUS4_GASTRO_ACID; // Edge case: Keep Gastro Acid if pokemon's ability can have effect after fainting, for example Innards Out.
    ...
    gProtectStructs[battler].petrified = FALSE;
...
static void TurnValuesCleanUp(bool8 var0)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (var0)
        {
            ...
            gProtectStructs[i].petrified = FALSE;
...
u32 GetDynamicMoveType(struct Pokemon *mon, u32 move, u32 battler, enum MonState state)
{
    ...
    else if (gMovesInfo[move].soundMove && ability == ABILITY_SOUSAPHONE)
    {
        return TYPE_STEEL;
    }
    else if (moveEffect == EFFECT_BRUTAL_PUNISHMENT && species == SPECIES_DISBEARY_EBIL)
    {
        return TYPE_DARK;
    }
