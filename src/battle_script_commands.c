...
static const u32 sStatusFlagsForMoveEffects[NUM_MOVE_EFFECTS] =
{
    ...
    [MOVE_EFFECT_RECHARGE_D]     = STATUS2_RECHARGE,
...
static void Cmd_attackcanceler(void)
{
    ...
    if (gSpecialStatuses[gBattlerAttacker].parentalBondState == PARENTAL_BOND_OFF
     && (GetBattlerAbility(gBattlerAttacker) == ABILITY_PARENTAL_BOND || GetBattlerAbility(gBattlerAttacker) == ABILITY_OVEREAGER)
     && IsMoveAffectedByParentalBond(gCurrentMove, gBattlerAttacker)
     && !(gAbsentBattlerFlags & (1u << gBattlerTarget))
     && GetActiveGimmick(gBattlerAttacker) != GIMMICK_Z_MOVE)
    {
        gSpecialStatuses[gBattlerAttacker].parentalBondState = PARENTAL_BOND_1ST_HIT;
        gMultiHitCounter = GetBattlerAbility(gBattlerAttacker) == ABILITY_OVEREAGER ? 3 : 2;
        ...
        || (IsMoveNotAllowedInSkyBattles(gCurrentMove)))
    {
        ...
        if (effect == EFFECT_FLING || EFFECT_FLAK_CANNON) // Edge case for removing a mon's item when there is no target available after using Fling.
     ...
     && effect != EFFECT_UPPER_HAND)
    {
        ...
        if (gSpecialStatuses[gBattlerAttacker].parentalBondState == PARENTAL_BOND_1ST_HIT)
        {
            ...
            gMultiHitCounter = 0;
        } else if (GetBattlerAbility(gBattlerAttacker) == ABILITY_OVEREAGER // Only applies for Overeager
            && gSpecialStatuses[gBattlerAttacker].parentalBondState == PARENTAL_BOND_2ND_HIT)
        {
            gSpecialStatuses[gBattlerAttacker].parentalBondState = PARENTAL_BOND_OFF; // No third hit if second hit was blocked
            gSpecialStatuses[gBattlerAttacker].multiHitOn = 0;
            gMultiHitCounter = 0;
        }
...
static void AccuracyCheck(bool32 recalcDragonDarts, const u8 *nextInstr, const u8 *failInstr, u16 move)
{
    ...
    if (move == NO_ACC_CALC_CHECK_LOCK_ON)
    {
        ...
        || gSpecialStatuses[gBattlerAttacker].parentalBondState == PARENTAL_BOND_3RD_HIT
...
static inline u32 GetHoldEffectCritChanceIncrease(u32 battler, enum ItemHoldEffect holdEffect)
{
    ...
    case HOLD_EFFECT_LUCKY_PUNCH:
        if (gBattleMons[battler].species == SPECIES_CHANSEY || gBattleMons[battler].species == SPECIES_FUCKER)
            critStageIncrease = 2;
        break;
...
static void Cmd_attackanimation(void)
{
    ...
    else
    {
        if (gSpecialStatuses[gBattlerAttacker].parentalBondState == PARENTAL_BOND_2ND_HIT
            || gSpecialStatuses[gBattlerAttacker].parentalBondState == PARENTAL_BOND_3RD_HIT) // No animation on second and third hits
        {
            gBattlescriptCurrInstr = cmd->nextInstr;
            return;
        }
...
void SetMoveEffect(bool32 primary, bool32 certain)
{
    ...
    {
    case MOVE_EFFECT_STICKY_TONGUE:
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_EVOCATION: // Evocation
                ...
                break;
            case MOVE_EFFECT_BRUTAL_PUNISHMENT: // Brutal Punishment
                if (!NoAliveMonsForEitherParty())
                {
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
            ...
            case MOVE_EFFECT_RECOIL_HP_25: // Struggle
                ...
                else if (GetBattlerAbility(gEffectBattler) == ABILITY_OVEREAGER)
                {
                    gBattleStruct->moveDamage[gEffectBattler] *= 3;
                }
...
static bool32 TryStickyTongueBattleScript(u32 battlerDef)
{
    if (gBattleMons[battlerDef].item != 0
        && CanBattlerGetOrLoseItem(battlerDef, gBattleMons[battlerDef].item)
        && !NoAliveMonsForEitherParty())
    {
        ...
        else
        {
            ...
            {
                gWishFutureKnock.knockedOffMons[side] |= 1u << gBattlerTarget;
            }

            BattleScriptPushCursor();
            gBattlescriptCurrInstr = BattleScript_StickyTongued;
        }
        return TRUE;
    }
    return FALSE;
...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_ABILITIES_ATTACKER: // Poison Touch, possibly other in the future
            if (AbilityBattleEffects(ABILITYEFFECT_BEAR_HUG_ATTACKER, gBattlerAttacker, 0, 0, 0))
                effect = TRUE;
            gBattleScripting.moveendState++;
            break;
        case BEAR_HUG_END_ABILITIES_ATTACKER: // Poison Touch, possibly other in the future
        ...
        case MOVEEND_FIRST_MOVE_BLOCK:
            ...
            case MOVE_EFFECT_STICKY_TONGUE:
                if (!DoesSubstituteBlockMove(gBattlerAttacker, gBattlerTarget, gCurrentMove))
                {
                    effect = TryStickyTongueBattleScript(gBattlerTarget);
                }
                break;
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_MEME_PUNCH:
    {
        ...
        if (bits)
        {
            ...
            {
                statId = (Random() % (NUM_BATTLE_STATS - 3)) + 1;
            } while (!(bits & 1u << statId) || (gBattleMons[battler].statStages[statId] != minimum));
    ...
    case VARIOUS_TRY_INSTRUCT:
    {
        ...
        if (move == MOVE_NONE || move == MOVE_UNAVAILABLE || MoveHasAdditionalEffectSelf(move, MOVE_EFFECT_RECHARGE) || MoveHasAdditionalEffectSelf(move, MOVE_EFFECT_RECHARGE_D)
    ...
    case VARIOUS_SET_MIRAGE_VEIL:
    {
        ...
        if (gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_MIRAGE_VEIL
            || !(HasWeatherEffect() && gBattleWeather & (B_WEATHER_SANDSTORM)))
        {
            gBattleStruct->moveResultFlags[gBattlerTarget] |= MOVE_RESULT_MISSED;
            gBattleCommunication[MULTISTRING_CHOOSER] = 0;
        }
        else
        {
            ...
            if (GetBattlerHoldEffect(battler, TRUE) == HOLD_EFFECT_LIGHT_CLAY)
                gSideTimers[GetBattlerSide(battler)].mirageVeilTimer = gBattleTurnCounter + 8;
            else
                gSideTimers[GetBattlerSide(battler)].mirageVeilTimer = gBattleTurnCounter + 5;
...
static void Cmd_setprotectlike(void)
{
     ...
     || (protectMethod == PROTECT_QUICK_GUARD && B_QUICK_GUARD != GEN_5))
    {
        ...
        else if (gCurrentMove == MOVE_PETRIFY)
        {
            gProtectStructs[gBattlerAttacker].petrified = TRUE;
...
void BS_TryDetonatorExplosion(void)
{
    ...
    gBattleStruct->moveDamage[gBattlerAttacker] = gBattleMons[gBattlerAttacker].hp;
    BtlController_EmitHealthBarUpdate(gBattlerAttacker, B_COMM_TO_CONTROLLER, gBattleMons[gBattlerAttacker].hp - 1);
    MarkBattlerForControllerExec(gBattlerAttacker);
    gBattlescriptCurrInstr = cmd->nextInstr;
}
...
static u32 ChangeStatBuffs(s8 statValue, u32 statId, u32 flags, const u8 *BS_ptr)
{
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        // Prevent user stat downs that are "certain" to happen from move effects (e.g. Superpower)
...
static bool32 InvalidDrumrollMove(u32 move)
{
    return gMovesInfo[move].effect == EFFECT_PLACEHOLDER
    || gMovesInfo[move].metronomeBanned
    || gMovesInfo[move].power < 110;
}

void BS_Drumroll(void)
{
    NATIVE_ARGS();
    ...
    gCurrentMove = RandomUniformExcept(RNG_DRUMROLL, 1, moveCount - 1, InvalidDrumrollMove);
    gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
    SetAtkCancellerForCalledMove();
    gBattlescriptCurrInstr = GetMoveBattleScript(gCurrentMove);
    gBattlerTarget = GetBattleMoveTarget(gCurrentMove, NO_TARGET_OVERRIDE);    
...
static void Cmd_settailwind(void)
{
    ...
    if (!(gSideStatuses[side] & SIDE_STATUS_TAILWIND))
    {
        ...
        if (gMovesInfo[gChosenMoveByBattler[gBattlerAttacker]].effect == EFFECT_BACKDRAFT)
        {
            gSideTimers[side].tailwindTimer = gBattleTurnCounter + 2;
            gBattlescriptCurrInstr = cmd->nextInstr;
        }
        else
        {
...
static void Cmd_trysetfutureattack(void)
{
    ...
    else
    {
        ...
        {
            gWishFutureKnock.futureSightCounter[gBattlerTarget] = gBattleTurnCounter + 4;
        }
        else
        {
