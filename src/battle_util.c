...
static void CancellerSupportive(u32 *effect)
{
    ...
    if (supportiveBattler && (GetMoveEffect(gCurrentMove) == EFFECT_RECOIL
...
static void CancellerPivotingStopSign(u32 *effect)
{
    if (GetBattlerAbility(gBattlerAttacker) == ABILITY_STOP_SIGN 
     && (gMovesInfo[gCurrentMove].effect == EFFECT_HIT_ESCAPE
     || gMovesInfo[gCurrentMove].effect == EFFECT_PARTING_SHOT))
    {
        gBattleScripting.battler = gBattlerAttacker;
        CancelMultiTurnMoves(gBattlerAttacker, SKY_DROP_ATTACKCANCELLER_CHECK);        
        gBattlescriptCurrInstr = BattleScript_StopsignStopsPivoting;
        gHitMarker |= HITMARKER_UNABLE_TO_USE_MOVE;
        *effect = 1;
    }
}

static void CancellerFoodUnnerve(u32 *effect)
{
    if(GetBattlerAbility(gBattlerAttacker) == ABILITY_UNNERVE && gMovesInfo[gCurrentMove].foodMove)
    {
        gBattleScripting.battler = gBattlerAttacker;
        CancelMultiTurnMoves(gBattlerAttacker, SKY_DROP_ATTACKCANCELLER_CHECK);        
...
static const MoveSuccessOrderCancellers sMoveSuccessOrderCancellers[] =
{
    ...
    [CANCELLER_RECOIL_SUPPORTIVE] = CancellerSupportive,
    [CANCELLER_PIVOTING_STOPSIGN] = CancellerPivotingStopSign,
    [CANCELLER_FOOD_UNNERVE] = CancellerFoodUnnerve,
};
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_HORROR:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattlerAttacker = battler;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_HorrorActivates);
                effect++;
            }
            break;
    ...
    case ABILITYEFFECT_ENDTURN:
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_HEALER:
                ...
                break;
            case ABILITY_DISPENSER:
            {
                if (!IsBattlerAtMaxHp(battler) && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK))
                {
                    BattleScriptPushCursorAndCallback(BattleScript_DispenserActivates);
                    gBattleStruct->moveDamage[battler] = GetNonDynamaxMaxHP(battler) / 16;
                    if (gBattleStruct->moveDamage[battler] == 0)
                        gBattleStruct->moveDamage[battler] = 1;
                    gBattleStruct->moveDamage[battler] *= -1;            
                ...
                gBattleScripting.battler = BATTLE_PARTNER(battler);
                if (IsBattlerAlive(gBattleScripting.battler) 
                    && !IsBattlerAtMaxHp(gBattleScripting.battler)
                    && !(gStatuses3[gBattleScripting.battler] & STATUS3_HEAL_BLOCK))
                    {
                        BattleScriptPushCursorAndCallback(BattleScript_DispenserActivatesPartner);
                        gBattleStruct->moveDamage[battler] = GetNonDynamaxMaxHP(gBattleScripting.battler) / 16;
                        if (gBattleStruct->moveDamage[battler] == 0)
                            gBattleStruct->moveDamage[battler] = 1;
                        gBattleStruct->moveDamage[battler] *= -1;    
                    }
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_TRANSFUSION:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             ...
             && !gAbilitiesInfo[gBattleMons[gBattlerAttacker].ability].cantBeSuppressed
             && !(gBattleMons[gBattlerAttacker].types[0] == gSpeciesInfo[gBattleMons[gBattlerTarget].species].types[0])
             && !(gBattleMons[gBattlerAttacker].types[1] == gSpeciesInfo[gBattleMons[gBattlerTarget].species].types[1]))
                {
                    gBattleMons[gBattlerAttacker].types[0] = gSpeciesInfo[gBattleMons[gBattlerTarget].species].types[0];
                    gBattleMons[gBattlerAttacker].types[0] = gSpeciesInfo[gBattleMons[gBattlerTarget].species].types[0];
                    BattleScriptPushCursor();
        ...
        case ABILITY_TETANUS:
            if (B_ABILITY_TRIGGER_CHANCE >= GEN_4 ? RandomPercentage(RNG_TETANUS, 100) : RandomChance(RNG_TETANUS, 1, 3))
            {
                ...
                && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
                && IsBattlerTurnDamaged(gBattlerTarget)
                && CanBePoisoned(gBattlerTarget, gBattlerAttacker, GetBattlerAbility(gBattlerAttacker), GetBattlerAbility(gBattlerTarget))
                && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_PROTECTIVE_PADS
                && IsMoveMakingContact(move, gBattlerAttacker))
                {
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        case ABILITY_POISON_TOUCH:
            ...
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             ...
             && IsMoveMakingContact(move, gBattlerAttacker)
             && IsBattlerTurnDamaged(gBattlerTarget) // Need to actually hit the target
             && RandomPercentage(RNG_CONFUSION, 100))
            {
        ...
        case ABILITY_ULTRAPOSITION:
            if ((gBattleMons[gBattlerAttacker].species == SPECIES_SHIRIBIKO)
    ...
    case ABILITYEFFECT_BEAR_HUG_ATTACKER:
        if (GetMoveArgType(move) == ARG_BEAR_HUG)
        {
            switch (gLastUsedAbility)
            {
                case ABILITY_POISON_POINT:
                ...
                case ABILITY_EFFECT_SPORE:
                {
                    ...
                    && GetBattlerHoldEffect(gBattlerTarget, TRUE) != HOLD_EFFECT_SAFETY_GOGGLES)
                    {
                        ...
                        i = RandomUniform(RNG_BEAR_HUG, 0, B_ABILITY_TRIGGER_CHANCE >= GEN_4 ? 99 : 299);
                        if (i < poison)
                            goto BEAR_HUG_POISON_POINT;
                        if (i < paralysis)
                            goto BEAR_HUG_STATIC;
                        // Sleep
                        if (i < sleep
                        && !(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
                        && IsBattlerAlive(gBattlerTarget)
                        && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
                        && IsBattlerTurnDamaged(gBattlerTarget)
                        && CanBeSlept(gBattlerAttacker, gBattlerTarget, ability, BLOCKED_BY_SLEEP_CLAUSE)
                        && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_PROTECTIVE_PADS
                        && IsMoveMakingContact(move, gBattlerAttacker))
                        {
                ...
                case ABILITY_CUTE_CHARM:
                    ...
                    && IsBattlerTurnDamaged(gBattlerTarget)
                    && IsBattlerAlive(gBattlerAttacker)
                    && (B_ABILITY_TRIGGER_CHANCE >= GEN_4 ? RandomPercentage(RNG_CUTE_CHARM, 1) : RandomChance(RNG_CUTE_CHARM, 1, 3))
                    && !(gBattleMons[gBattlerTarget].status2 & STATUS2_INFATUATION)
                    && AreBattlersOfOppositeGender(gBattlerTarget, gBattlerAttacker)
                    ...
                    && !IsAbilityOnSide(gBattlerTarget, ABILITY_AROMA_VEIL))
                    {
                        ...
                        BattleScriptPushCursor();
                        gBattlescriptCurrInstr = BattleScript_BearHugCuteCharmActivates;
                        effect++;
                    }
                    break;
                case ABILITY_GOOEY:
                ...
                case ABILITY_MUMMY:
                    ...
                    && !gAbilitiesInfo[gBattleMons[gBattlerTarget].ability].cantBeSuppressed)
                    {
                        ...
                        gLastUsedAbility = gBattleMons[gBattlerTarget].ability = gDisableStructs[gBattlerTarget].overwrittenAbility = gBattleMons[gBattlerAttacker].ability;
                        BattleScriptPushCursor();
                        gBattlescriptCurrInstr = BattleScript_BearHugMummyActivates;
                        effect++;
                        break;
                    }
                    break;
                case ABILITY_PICKPOCKET:
                {
                    if (gBattleMons[gBattlerAttacker].item == ITEM_NONE
                     ...
                     && !(gWishFutureKnock.knockedOffMons[GetBattlerSide(gBattlerTarget)] & 1u << gBattlerTarget)
                ...
                case ABILITY_IRON_BARBS:
                    ...
                    && IsMoveMakingContact(move, gBattlerAttacker))
                    {
                        gBattleStruct->moveDamage[battler] = GetNonDynamaxMaxHP(gBattlerTarget) / (B_ROUGH_SKIN_DMG >= GEN_4 ? 8 : 16);
                        if (gBattleStruct->moveDamage[battler] == 0)
                            gBattleStruct->moveDamage[battler] = 1;
                        PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gBattleMons[gBattlerAttacker].ability);
                        BattleScriptPushCursor();
...
u32 ItemBattleEffects(enum ItemCaseId caseID, u32 battler, bool32 moveTurn)
{
    ...
    case ITEMEFFECT_KINGSROCK:
        ...
        case HOLD_EFFECT_ROCKY_HELMET:
                if (GetMoveArgType(gCurrentMove) == ARG_BEAR_HUG)
                {
                    if (IsBattlerTurnDamaged(gBattlerTarget)
                    ...
                    {
                        gBattleStruct->moveDamage[battler] = GetNonDynamaxMaxHP(gBattlerTarget) / 6;
                        if (gBattleStruct->moveDamage[battler] == 0)
                            gBattleStruct->moveDamage[battler] = 1;
                        effect = ITEM_HP_CHANGE;
                        BattleScriptPushCursor();
...
enum ItemHoldEffect GetBattlerHoldEffectInternal(u32 battler, bool32 checkNegating, bool32 checkAbility)
{
    if (checkNegating)
    {
        ...
        if (gFieldStatuses & STATUS_FIELD_MAGIC_ROOM)
            return HOLD_EFFECT_NONE;
        if (checkAbility && GetBattlerAbility(battler) == ABILITY_KLUTZ && !(gStatuses3[battler] & STATUS4_GASTRO_ACID))
            return HOLD_EFFECT_NONE;
    }
...
bool32 IsBattlerProtected(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (IsSideProtected(battlerDef, PROTECT_CRAFTY_SHIELD)
     ...
     && GetMoveEffect(move) != EFFECT_COACHING)
        ...
        isProtected = TRUE;
    else if (gProtectStructs[battlerDef].petrified && gMovesInfo[move].power != 0)
        isProtected = TRUE;
    ...
    case EFFECT_DOUBLE_POWER_ON_ARG_STATUS:
        ...
        if ((gBattleMons[battlerDef].status1 | (STATUS1_SLEEP * (abilityDef == ABILITY_COMATOSE))) & GetMoveEffectArg_Status(move)
         && !((GetMoveAdditionalEffectById(move, 0)->moveEffect == MOVE_EFFECT_REMOVE_STATUS) && DoesSubstituteBlockMove(battlerAtk, battlerDef, move)))
            basePower *= 2;
        break;
    case EFFECT_TRIPLE_POWER_ON_ARG_STATUS:
        // Comatose targets treated as if asleep
        if ((gBattleMons[battlerDef].status1 | (STATUS1_SLEEP * (abilityDef == ABILITY_COMATOSE))) & GetMoveEffectArg_Status(move)
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, enum ItemHoldEffect holdEffectAtk, u32 weather)
{
    ...
    case ABILITY_PROTOSYNTHESIS:
        {
            ...
            if (((weather & B_WEATHER_SUN && HasWeatherEffect()) || gDisableStructs[battlerDef].boosterEnergyActivates)
             && ((IsBattleMovePhysical(move) && atkHighestStat == STAT_ATK) || (IsBattleMoveSpecial(move) && atkHighestStat == STAT_SPATK))
    ...
    if (GetActiveGimmick(battlerAtk) == GIMMICK_TERA
        ...
        && moveEffect != EFFECT_SHINE_STRIKE
...
static inline u32 CalcAttackStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, enum ItemHoldEffect holdEffectAtk, u32 weather)
{
    ...
    else if (moveEffect == EFFECT_BODY_PRESS || moveEffect == EFFECT_TOMBSTONER)
    {
        if (IsBattleMovePhysical(move))
        {
    ...
    case ABILITY_SOLAR_POWER:
        ...
        break;
    case ABILITY_SHOWER_POWER:
        if (IsBattleMoveSpecial(move) && IsBattlerWeatherAffected(battlerAtk, B_WEATHER_RAIN))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    ...
    case HOLD_EFFECT_THICK_CLUB:
        if ((atkBaseSpeciesId == SPECIES_BANDICUTE || atkBaseSpeciesId == SPECIES_NAUGHTYCOOT) && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    case HOLD_EFFECT_DEEP_SEA_TOOTH:
        if (gBattleMons[battlerAtk].species == SPECIES_CLAMPERL && IsBattleMoveSpecial(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    case HOLD_EFFECT_CURSED_FANG:
        if (gBattleMons[battlerAtk].species == SPECIES_WINKHULU && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    case HOLD_EFFECT_CRIMSON_LENS:
        if (gBattleMons[battlerAtk].species == SPECIES_WINKHULU && IsBattleMoveSpecial(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
...
static inline uq4_12_t GetParentalBondModifier(u32 battlerAtk)
{
    if (gSpecialStatuses[battlerAtk].parentalBondState != PARENTAL_BOND_2ND_HIT
        && gSpecialStatuses[battlerAtk].parentalBondState != PARENTAL_BOND_3RD_HIT)
        return UQ_4_12(1.0);
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defAbility, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    if (GetMoveEffect(move) == EFFECT_SUPER_EFFECTIVE_ON_ARG && defType == GetMoveArgType(move))
        mod = UQ_4_12(2.0);
    if (gMovesInfo[move].effect == EFFECT_NEUTRAL_EFFECTIVE_ON_ARG && defType == GetMoveArgType(move))
        mod = UQ_4_12(1.0);
...
{
    if ((GetMoveEffect(move) == EFFECT_HIT_ENEMY_HEAL_ALLY || EFFECT_HIT_TAKE_UR_MEDS)
